# Copyright 2024 IBM Corporation
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
"""Blocker 6 front-half attention discriminator.

Compare a two-call shared HOP body with a one-call inline body for only:

    Q/K/V projections -> RoPE(Q/K) -> KV-cache writes

The region returns Q and mutates the cache inputs.  The harness restores the
same device cache tensors to their call-1 entry state before N=1, then compares
Q and both mutated caches independently.  No RNG seed is used.

Run from the hf-adapters worktree:

    PYTHONPATH="$PWD" HF_HOME=/mnt/models/hf_cache/ \
        python repros/hier/standalone_qkv_discriminator.py
"""

import math

import torch
from torch.compiler import nested_compile_region

from hf_adapters.auto_spyre_model import torch_dtype_for_model_path
from hf_adapters.hf_common import (
    DEVICE,
    allocate_kv_caches,
    apply_rope_matmul,
    build_prefill_mask,
    generation_cache_len,
    get_backbone,
    get_model_dtype,
    kv_cache_update,
    move_model_to_spyre,
)
from tests.conftest import load_ref_model, resolve_adapter_module_for_test

MODEL = "ibm-granite/granite-3.3-2b-instruct"
BLOCK_SIZE = 64


@nested_compile_region
def _shared_qkv(
    attn,
    hidden_states,
    selected_freqs,
    key_cache,
    value_cache,
):
    bsz, seq_len, _ = hidden_states.shape
    q = (
        attn.q_proj(hidden_states)
        .view(bsz, seq_len, -1, attn.head_dim)
        .transpose(1, 2)
    )
    k = (
        attn.k_proj(hidden_states)
        .view(bsz, seq_len, -1, attn.head_dim)
        .transpose(1, 2)
    )
    v = (
        attn.v_proj(hidden_states)
        .view(bsz, seq_len, -1, attn.v_head_dim)
        .transpose(1, 2)
    )
    q = apply_rope_matmul(q, selected_freqs)
    k = apply_rope_matmul(k, selected_freqs)
    kv_cache_update(k, v, key_cache, value_cache, False, 0, 0)
    return q


def main():
    from transformers import AutoTokenizer

    adapter = resolve_adapter_module_for_test(MODEL)
    tokenizer = AutoTokenizer.from_pretrained(MODEL)
    model = load_ref_model(model_path=MODEL, adapter_mod=adapter)

    input_ids = tokenizer(
        "The capital of France is", return_tensors="pt"
    )["input_ids"]
    seq_len = input_ids.shape[1]
    batch_size = 1

    dtype = torch_dtype_for_model_path(MODEL)
    move_model_to_spyre(model=model, module=adapter, dtype=dtype)
    mdtype = get_model_dtype(model)

    padded_len = math.ceil(seq_len / BLOCK_SIZE) * BLOCK_SIZE
    prompt_offset = padded_len - seq_len
    if prompt_offset:
        pad = input_ids.new_zeros((batch_size, prompt_offset))
        padded_ids = torch.cat([pad, input_ids], dim=1)
    else:
        padded_ids = input_ids

    position_ids = torch.zeros((batch_size, padded_len), dtype=torch.long)
    position_ids[:, prompt_offset:] = torch.arange(seq_len)
    max_cache_len = generation_cache_len(seq_len, 5)
    mask = build_prefill_mask(
        batch_size,
        padded_len,
        max_cache_len,
        prompt_offset,
        dtype=mdtype,
    ).to(DEVICE)

    dev_ids = padded_ids.to(DEVICE)
    dev_pos = position_ids.to(DEVICE)
    freqs = model._spyre_rope(dev_ids, dev_pos)

    backbone = get_backbone(model)
    block = list(backbone.layers)[0]
    attn = block.self_attn
    with torch.no_grad():
        h0 = backbone.embed_tokens(dev_ids) * backbone.embedding_multiplier
        norm_fn = torch.compile(block.input_layernorm, dynamic=False)
        h_norm = norm_fn(h0)

    kcs2, vcs2 = allocate_kv_caches(
        model, batch_size, max_cache_len, mdtype
    )
    kc0, vc0 = kcs2[0], vcs2[0]
    kc0_entry = kc0.to("cpu").clone()
    vc0_entry = vc0.to("cpu").clone()

    print("\n===N2=== two QKV region calls -> shared HOP body", flush=True)

    def two_calls(h, selected_freqs, kcs, vcs):
        q1 = _shared_qkv(attn, h, selected_freqs, kcs[0], vcs[0])
        q2 = _shared_qkv(
            attn, h * 1.0, selected_freqs, kcs[1], vcs[1]
        )
        return q1, q2

    n2_fn = torch.compile(two_calls, dynamic=False)
    with torch.no_grad():
        n2_q1, _n2_q2 = n2_fn(h_norm, freqs, kcs2, vcs2)
    n2_q_host = n2_q1.to("cpu").float()
    n2_kc_host = kc0.to("cpu").float()
    n2_vc_host = vc0.to("cpu").float()

    with torch.no_grad():
        kc0.copy_(kc0_entry.to(DEVICE))
        vc0.copy_(vc0_entry.to(DEVICE))

    print("\n===N1=== one QKV region call -> inline body", flush=True)

    def one_call(h, selected_freqs, kc, vc):
        return _shared_qkv(attn, h, selected_freqs, kc, vc)

    n1_fn = torch.compile(one_call, dynamic=False)
    with torch.no_grad():
        n1_q = n1_fn(h_norm, freqs, kc0, vc0)
    n1_q_host = n1_q.to("cpu").float()
    n1_kc_host = kc0.to("cpu").float()
    n1_vc_host = vc0.to("cpu").float()

    q_delta = (n2_q_host - n1_q_host).abs().max().item()
    kc_delta = (n2_kc_host - n1_kc_host).abs().max().item()
    vc_delta = (n2_vc_host - n1_vc_host).abs().max().item()
    print(f"\n=== max|Q _1(call1) - _0(call1)| = {q_delta:.6f} ===")
    print(f"=== max|K-cache _1(call1) - _0(call1)| = {kc_delta:.6f} ===")
    print(f"=== max|V-cache _1(call1) - _0(call1)| = {vc_delta:.6f} ===")

    failing = [
        name
        for name, delta in (
            ("Q projection/RoPE", q_delta),
            ("K projection/RoPE/cache write", kc_delta),
            ("V projection/cache write", vc_delta),
        )
        if delta > 0.5
    ]
    if failing:
        print(
            ">>> Front half reproduces Blocker 6 in: " + ", ".join(failing),
            flush=True,
        )
    else:
        print(
            ">>> Front half is clean: the defect requires SDPA/output "
            "projection or cross-boundary fusion.",
            flush=True,
        )


if __name__ == "__main__":
    main()
