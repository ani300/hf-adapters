...
from hf_adapters.hf_gemma4 import _Gemma4BlockSpec, _make_block_forward
...
def test_kv_sharing_sliding_layer_matches_the_band_masked_path():
    ...
    op = _make_block_forward(op_spec)

    state = (
        torch.randn(hidden_size, hidden_size),  # q
        torch.randn(hidden_size, hidden_size),  # o
        torch.ones(HEAD_DIM),  # q norm
        torch.randn(intermediate_size, hidden_size),  # gate
        torch.randn(intermediate_size, hidden_size),  # up
        torch.randn(hidden_size, intermediate_size),  # down
        torch.ones(hidden_size),  # input norm
        torch.ones(hidden_size),  # post-attention norm
        torch.ones(hidden_size),  # pre-FFN norm
        torch.ones(hidden_size),  # post-FFN norm
        torch.tensor(1.0),  # layer scalar
    )
