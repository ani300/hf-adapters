...
def _copy_default(shard, *, device, dtype):
    from torch_spyre.model_utils import dma_tensor_to_spyre

    return dma_tensor_to_spyre(shard, target_dtype=dtype, device=device)


def _copy_linear(shard, *, device, dtype):
    from torch_spyre.model_utils import dma_linear_weight_to_spyre

    if shard.ndim == 2:
        return dma_linear_weight_to_spyre(shard, target_dtype=dtype, device=device)
...
def _copy_embedding(shard, *, device, dtype):
    ...
    if shard.ndim == 2:
        moved = dma_embedding_weight_to_spyre(shard, target_dtype=dtype, device=device)
...
class SpyreEmbeddingRowwiseParallel(_SpyreEmbeddingParallel):
    ...
    def _prepare_input_fn(self, mod, inputs, device_mesh):
        ...
        vocab_start = rank * per_partition_size
        vocab_end = vocab_start + per_partition_size
    ...
    def prepare_module_tp(self, module, device_mesh, **kwargs):
        ...
        def prepare_inputs(mod, inputs):
            return self._prepare_input_fn(mod, inputs, device_mesh)
...
def _matches_plan(parameter_name: str, plan: Mapping[str, str]) -> bool:
    # Match Transformers' build_glob_alternation behavior: '*' expands to
    # '.*', patterns are searched (not anchored), and other characters are
    # left as regex syntax.
    return any(
        re.search(pattern.replace("*", ".*"), parameter_name) for pattern in plan
    )
