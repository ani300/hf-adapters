...
Expected: 4 passed (two prefill shapes, decode, left padding).

Diagnosis if it fails:

- **`test_left_padding` alone fails**, others pass → suspect
  `window_band_mask`'s `float("-inf")`. Host fp16 `-inf` lands on device as a
  finite `-3.35e7`, roughly 500x past fp16's max finite, which round-trips badly
  through fp16 materializations; `hf_common._mask_fill_value` uses
  `finfo(dtype).min / 2` for exactly this reason and cites the batch>1 Qwen3
  decode bug. Fix: use `torch.finfo(dtype).min / 2` in `window_band_mask` instead
  of `float("-inf")`, in the torch-spyre repo, and re-run Task 2's suite.
- **Everything fails at `head_dim=256`** → this is Task 1's finding arriving late;
  re-run Task 1's tests and report.
- **Only `seqlen_q=512` prefill fails** → a `read_start` / `q_kv_offset` mismatch
  across the 8 query blocks. Print `plan.read_start(i)` for each block and compare
  against the band's expectation.
...
def test_anchored_decode_matches_band_mask_across_a_shift():
    ...
    band_k, band_v = _caches(1, full_capacity)
    op_k, op_v = _caches(1, prompt)
    ...
    for step_index in range(steps):
        ...
        torch.testing.assert_close(
            ...
            rtol=RTOL,
            atol=ATOL,
            msg=f"step {step_index} (shift={step.do_shift})",
        )
